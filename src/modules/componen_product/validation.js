...
const createValidation = [
  body("componen_type")
    ...
    .isInt({ min: 1, max: 5 })
    .withMessage(
      "Componen type harus berupa angka 1, 2, 3, 4, atau 5 (1: OFF ROAD REGULAR, 2: ON ROAD REGULAR, 3: OFF ROAD IRREGULAR, 4: OFF ROAD REGULAR EV, 5: ON ROAD REGULAR EV)",
    ),
  body("company_name")
    .optional()
    .isLength({ max: 255 })
  ...
  body("product_type")
    ...
    .custom((value) => {
      ...
      if (typeof value === "string") {
        ...
        if (!validTypes.includes(trimmed)) {
          throw new Error(
            "Product type harus berupa unit, non_unit, hardware, implementation, atau application",
          );
        }
        return true;
      }
    ...
    .withMessage(
      "Product type harus berupa unit, non_unit, hardware, implementation, atau application (maksimal 255 karakter)",
    ),
    ...
    .custom((value) => {
      ...
      if (typeof parsed === "string") {
        ...
        try {
          ...
          throw new Error(
            "Format componen_product_specifications harus berupa JSON yang valid",
          );
        }
      }
      ...
      if (!Array.isArray(parsed)) {
        if (typeof parsed !== "object" || parsed === null) {
          throw new Error(
            "componen_product_specifications harus berupa objek atau array objek",
          );
        }
        parsed = [parsed];
      }
      ...
      parsed.forEach((item) => {
        if (typeof item !== "object" || item === null) {
          throw new Error(
            "Setiap item componen_product_specifications harus berupa objek",
          );
        }
      });
...
const updateValidation = [
  ...
  body("company_name")
    ...
    .withMessage(
      "Company name harus berupa string, string kosong, null, atau NaN (maksimal 255 karakter)",
    ),
  body("product_type")
    .optional()
    .custom((value) => {
...
const listValidation = [
  ...
  body("sort_by")
    ...
    .withMessage(
      "Sort by harus salah satu dari: created_at, code_unique, componen_product_name, segment, msi_model, msi_product, volume, componen_product_unit_model, product_type",
    ),
  body("sort_order")
    .optional()
    .isIn(["asc", "desc"])
